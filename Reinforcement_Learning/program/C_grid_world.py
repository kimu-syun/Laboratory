...
class Agent():
    ...
    GAMMA = 0.9
    ACTIONS = ["0", "1", "2", "3", "4"]
    act_dict = {'0':np.array([2,-2]), '1':np.array([1,-1]), '2':np.array([0,0]),\
                '3':np.array([-1,1]), '4':np.array([-2,2])}
    ...
    def reward(self, state, next_state, action, sensory):
        ...
        if sensory == 0:
            for i in range(0, 5):
                r_map.append(np.arange(i/10, i/10 + 5, 1))
        elif sensory == 1:
            for i in range(-1, 4):
                r_map.append(np.arange(i/10 -1, i/10 + 4, 1))
        else:
            for i in range(0, 5):
                r_map.append(np.full(5, i))

        # r = s' - s
        r = r_map[next_state[0]][next_state[1]] - r_map[state[0]][state[1]]
