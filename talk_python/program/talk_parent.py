...
def belief_sensory_distribution_make(likelihood_distribution, belief_hiddenstate_distribution, belief_sensory_distribution, epoch):
    for a in range(0, action_range):
        ...
        np.savetxt(f"./data/parent/belief_sensory_distribution/epoch_{epoch}_action_{a}.csv", belief_sensory_distribution[a], fmt="%.5f")
...
def epistemic_value_calculate(belief_hiddenstate_distribution, belief_sensory_distribution, belief_conditional_hiddenstate_distribution, epistemic_value):
    ...
    np.savetxt(f"./data/parent/epistemic_value.csv", epistemic_value, fmt="%.5f")
...
def predicted_surprised_calculate(belief_sensory_distribution, preference_distribution, predicted_surprised):
    ...
    np.savetxt(f"./data/parent/predicted_surprised.csv", predicted_surprised, fmt="%.5f")
...
def F_expected_calculate(epistemic_value, predicted_surprised, F_expected):
    ...
    np.savetxt(f"./data/parent/F_expected.csv", F_expected, fmt="%.5f")
    return F_expected


# q(x)を更新 q(x|y,a)->q(x)
def qxUpdate(belief_hiddenstate_distribution, belief_conditional_hiddenstate_distribution, y_signal, a, epoch):
    np.savetxt(f"./data/parent/belief_hiddenstate_distribution/epoch_{epoch}_action_{a}.csv", belief_hiddenstate_distribution[a], fmt="%.5f")
    for x in range(0, hidden_state_range):
...
def parent_inference(parent, epoch):
    ...
    # FE計算
    parent.epistemic_value = epistemic_value_calculate(parent.belief_hiddenstate_distribution, parent.belief_sensory_distribution, parent.belief_conditional_hiddenstate_distribution, parent.epistemic_value)
    parent.predicted_surprised = predicted_surprised_calculate(parent.belief_sensory_distribution, parent.preference_distribution, parent.predicted_surprised)
    parent.F_expected = F_expected_calculate(parent.epistemic_value, parent.predicted_surprised, parent.F_expected)
