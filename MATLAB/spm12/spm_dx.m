...
% exponentials
% f = dF/dx (および dfdx = dF/dxdx) のとき、dxはFに対するGauss-Newton上昇から
% の更新を表します。これは、{}を指定することによって正則化することができます。
% 重い正則化はt = -4、軽い正則化はt = 4に対応します。このバージョンのspm_dxは、
% 必要な行列指数計算を行うために、拡張系とPade近似を使用します。
%
% references:
%
...
% EXPV は、クリロフ部分空間投影法を用いて、一般行列 A に対して w = exp(t*A)*v の
% 近似値を計算します。行列の指数演算を単独で計算するのではなく、オペランドベクトル
% に対する指数演算子の作用を直接計算するのです。この方法によって、大規模な
% スパース問題に対応することができます。対象となる行列は、行列-ベクトル積を介して
% のみ相互作用します（行列なし法）。
%
%  w = expv( t, A, v )
...
%  reference below).
% これは行列指数問題の条件付けの指標として使われる。しかし、ハンプが大きくても
% (ハンプは上限)、特にハンプと｜w(t)｜/｜v｜が同じ大きさの場合、解は比較的かなり
% 正確になります(詳細は後述する を参照)。
%
%  Example 1:
%  ----------
