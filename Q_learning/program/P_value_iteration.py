...
from program import P_grid_world


def parent_DP(state, sensor):
    # agentの生成
    agent = P_grid_world.Agent(state)
        ...
        while(True):
            ...
            for i in range(num_row):
                ...
                #print("delta %f" % delta)
                    ...
                    tmp[index] =  (agent.reward(s,s_dash,action,sensor) + agent.GAMMA * V[s_dash[0]])
                V[i] = max(tmp)
    ...
    return next_state, pi[state[0]]

    ## 結果をグラフィカルに表示
    #方策を矢印で表示
    # P_grid_world.pi_arrow_plot(pi)
    #状態価値関数を表示
    # P_grid_world.V_value_plot(V)